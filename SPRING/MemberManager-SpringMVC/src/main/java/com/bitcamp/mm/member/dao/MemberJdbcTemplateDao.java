...
public class MemberJdbcTemplateDao {
	...
	public MemberInfo selectMemberById2(String id) {
		...
		try {
			memberInfo = template.queryForObject(sql, new Object[] { id }, new MemberRowMapper());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int insert(MemberInfo mi) {
		...
		return template.update(sql, mi.getId(), mi.getPw(), mi.getName(), mi.getPhoto());
	}
	...
	public int editMember(MemberInfo mi) {
	
		String sql = "update member set name=?, pw=?, photo=? where idx=?";

		return template.update(sql, mi.getName(), mi.getPw(), mi.getPhoto(), mi.getIdx());
	}
}
